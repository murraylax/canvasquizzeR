...
You can use the `readr::write_csv()` to write the resulting quiz to a .csv file, or `writexl::write_xlsx()` to write to an Excel .xlsx file:
```
# CSV file
readr::write_csv(quiz.df, file = "./quiz.csv")

# Excel file
writexl::write_xlsx(quiz.df, path = "./quiz.xlsx")
```
