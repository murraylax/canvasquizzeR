...
output: 
  pdf_document:
    template: "`r paste0(path.package('canvasquizzeR'),'quiz-template.tex')`"
    keep_md: true
    keep_tex: true
params:
  quiz.df: NA
...
clean_question <- function(s) {
  ...
  if(length(mm)!=0) {
    for(r in 1:nrow(mm)) {
      ...
      s <- stringr::stringr::str_replace(s, mm[r,1], latex)    
    }
  }
...
```{r}
if(!is.na(quiz.df)) { # Don't do anything if the quiz is NA
                  
  # Multiple Choice Questions
  
  df.mc <- filter(df, str_simplify(`Question Type`)=="mc")
  ...
  if(ngroups>0) {
    ...
    for(ng in 1:ngroups) {
      group.df <- filter(df.mc, G==groups[ng])
  ...
  df.essay <- filter(df, str_simplify(`Question Type`)=="essay")
  ...
  if(ngroups>0) {
    ...
    for(ng in 1:ngroups) {
      group.df <- filter(df.essay, G==groups[ng])
      print_random_questions_group(group.df, 1, params$version, includeanswers=params$includeanswers)
    }
