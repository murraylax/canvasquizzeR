...
clean_question <- function(s) {
  ...
  if(length(mm)!=0) {
    for(r in 1:nrow(mm)) {
      imgfile <- stringr::str_squish(mm[r,2])
      if(stringr::str_detect(imgfile, "http://") ||  stringr::str_detect(imgfile, "https://")) {
        syswget <- sprintf("wget %s", imgfile)
        system(syswget)
        imgfile <- stringr::str_match(imgfile, "[^/]+$")
      }
      
      latex <- sprintf("\n\n\\\\includegraphics[width=0.5\\\\linewidth]{%s}\n\n", imgfile)
      s <- stringr::str_replace(s, mm[r,1], latex)    
    }
  }
